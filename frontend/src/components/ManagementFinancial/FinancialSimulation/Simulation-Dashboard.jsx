...
const SimulationDashboard = ({
    ...
    const completedIncome = simulations
        .filter(s => s.type === 'income' && s.status === 'completed')
        .reduce((sum, s) => sum + parseFloat(s.amount), 0);

    const completedExpense = simulations
        .filter(s => s.type === 'expense' && s.status === 'completed')
        .reduce((sum, s) => sum + parseFloat(s.amount), 0);

    const plannedIncome = simulations
        .filter(s => s.type === 'income' && s.status === 'planned')
        .reduce((sum, s) => sum + parseFloat(s.amount), 0);

    const plannedExpense = simulations
        .filter(s => s.type === 'expense' && s.status === 'planned')
        .reduce((sum, s) => sum + parseFloat(s.amount), 0);
            ...
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-gray-600 dark:text-gray-400">
                        Kelola dan pantau simulasi keuangan Anda
                    </p>
                </div>
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                    + {formatCurrency(cashFlowSummary.summary.planned_income)} rencana
                                </p>
                            </div>
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-2xl font-bold text-red-600 dark:text-red-400 mt-1">
                                    {formatCurrency(cashFlowSummary.summary.total_expense)}
                                </p>
                                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                    + {formatCurrency(cashFlowSummary.summary.planned_expense)} rencana
                                </p>
                            </div>
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className={`text-2xl font-bold mt-1 ${
                                    cashFlowSummary.summary.net_cash_flow >= 0 
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-lg p-6 border border-gray-200 dark:border-gray-700">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className={`text-2xl font-bold mt-1 ${
                                    cashFlowSummary.summary.projected_net_cash_flow >= 0 
                                        ? 'text-green-600 dark:text-green-400' 
                                        : 'text-red-600 dark:text-red-400'
                                }`}>
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-2xl font-bold text-yellow-600 dark:text-yellow-400">
                                {simulations.filter(s => s.status === 'planned').length}
                            </p>
                        </div>
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        >
                                            {simulation.type === 'income' ? (
                                                <TrendingUp size={16} style={{ color: simulation.category?.color || '#6B7280' }} />
                                            ) : (
                                                <TrendingDown size={16} style={{ color: simulation.category?.color || '#6B7280' }} />
                                            )}
                                        </div>
                                        <div>
